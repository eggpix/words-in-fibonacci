...
Run [`find_words.py`](src/find_words.py) to generate a file `fibonacci_words.txt` in the root directory containing the words found. You can modify the number of Fibonacci elements checked by changing the `LOOP_NUM` variable. By default, words shorter than 5 characters are excluded, but this can be changed by modifying `MIN_WORD_LENGTH`.
